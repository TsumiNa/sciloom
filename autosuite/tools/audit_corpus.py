    ...
    for kind, expected in (("app", 68), ("asfp", 58)):
        ...
        with zipfile.ZipFile(archive_path) as archive:
            for member in sorted(archive.namelist()):
                ...
                members.append(
                    {
                        "archive": archive_path.relative_to(BASE).as_posix(),
                        "member": member,
                        "path": by_hash[key],
                        "bytes": str(len(raw)),
                        "sha256": key,
        ...
        for entries in (catalog, csv_catalog):
            check({r["file"] for r in entries} == {p.name for p in files}, f"{kind} catalog filenames differ")
...
def inventory():
    ...
    for path in sorted(BASE.rglob("*")):
        ...
        raw = path.read_bytes()
        records.append({"path": path.relative_to(BASE).as_posix(), "bytes": str(len(raw)), "sha256": digest(raw)})
    return records
