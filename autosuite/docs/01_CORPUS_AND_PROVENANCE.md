...
- `autosuite/corpus/archives/app_type_files.zip`: the unchanged original archive, 68 APP entries.
- `autosuite/corpus/archives/asfp_type_files.zip`: the unchanged original archive, 59 ASFP entries.
- `autosuite/corpus/app/`: all 68 distinct APP files, each gzip-compressed XML.
- `autosuite/corpus/asfp/`: 58 byte-distinct ASFP files. The archive entry
  `GetAspirateChunkVol - Copy.asfp` is identical to `GetAspirateChunkVol.asfp`;
  only the latter is expanded on disk.
- `autosuite/corpus/manual/1001.9796.T AutoSuite Software Manual.pdf`: the supplied manual.
...
`autosuite/corpus/app/config20260909_polymerization.app` is the primary current application.
`autosuite/corpus/extracted/latest_app/application.xml` matches its decompressed bytes.
The 52 single-function packages under `autosuite/corpus/extracted/latest_app/functions/`
...
- `autosuite/corpus/MANIFEST.csv`: current relative paths, sizes and SHA-256 hashes for
  reference files, derived documentation and tools; excludes itself and caches.
- `autosuite/corpus/catalogs/relocation.csv`: original paths and unchanged content hashes
  for relocated evidence. Old paths here are historical identifiers, not live links.
- `autosuite/corpus/catalogs/archive_members.csv`: each original APP/ASFP archive member
  mapped to its current expanded file, including the duplicate archive alias.
- `autosuite/corpus/catalogs/app_catalog.*` and `asfp_catalog.*`: current expanded-file catalogs.
