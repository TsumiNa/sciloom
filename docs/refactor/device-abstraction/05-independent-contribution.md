...
Add an independent test device with gain configuration, an additional registered command and a recording Target. Demonstrate schema extension, required multi-parameter configuration, typed arguments and gain range validation using public contributor interfaces. The added device itself must require no shared DSL/core edits.
...
Run shared acceptance; verify native property/command processing, unsupported-device diagnostics, contract mismatch and parameter errors, required configuration, and independent target/import typing. Keep developer output documentation next to its source.
