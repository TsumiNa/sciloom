...
| A03 Zone | F24 dynamic Stir; F30/F34/F48, primary APP layout; [value/layout mapping](../../../autosuite/docs/29_ZONE_VALUES_AND_LAYOUT.md), [selection evidence](../../../autosuite/docs/32_DYNAMIC_DEVICE_LOCATIONS.md) | 3.6.19 pp. 73–74, 3.8.5 p. 113, 3.9.2–3, 3.11.5 | Stages 13/16: values, APP extraction, candidate ancestry, captured/inherited scopes and independent physical snapshots implemented | Native union order/name/call copies pending. WellName/nonempty literals and DeviceAt/candidate emission rejected. Reliable failure propagation and native cross-call selection transport remain gated |
| A04 traversal | F43 and F46 sequential macros; [mapping](../../../autosuite/docs/30_ZONE_TRAVERSAL_MAPPING.md) | 3.8.10–12 pp. 121–126 | Implemented stage 14: indexing, captured iteration, complete groups, persistent targets, nested loops and failure-before-body checks | Size-one native macro emitted with private source/iterator and outer empty guard. Native capture/copy, order and empty behavior need Executor; indexing/groups >1 explicitly rejected pending failure propagation |
| A05 read | F25/F28; observed importrowmode 0/header 0 and mode 1/header 1; [mapping and 14 native probes](../../../autosuite/docs/27_CSV_READ_MAPPING.md) | 3.7.8 pp. 88–92 explicitly evaluates cells and truncates real-to-int values | Implemented stage 11: typed literal parsing, explicit files, defaults/statuses, atomic results, Python/IR/JSON tests | Native result aggregation, units/defaults, expression versus literal conversion, quote/encoding/newline profile and fatal propagation remain pending. AutoSuite rejects ReadCsv until equivalence is established |
| A06 append | F46, one text column, one observed export profile; [mapping and six native probes](../../../autosuite/docs/28_CSV_APPEND_MAPPING.md) | 3.7.9 pp. 93–96; XML exportbehaviour=0 not tied to its UI label | Implemented stage 12: capture, repeated writes, statuses, partial failure, Python/IR/JSON and explicit files | Append/new-file mapping, IO error, repeated rows and exact bytes pending. AutoSuite rejects both forms until mode and encoding are confirmed; default-fatal also needs the failure gate |
| A07 property | Primary APP single-value SetProperty, F45/F46/F50 user-property tasks; [mapping](../../../autosuite/docs/31_WELL_PROPERTY_MAPPING.md) | 3.7.6–7 pp. 85–87: single-well reads, missing/type fallback, same-value multiwell writes | Implemented stage 15: explicit store/directory, strict/default reads, captured writes, immutable events, Python/IR/JSON parity | Native text/default behavior, empty guard and call behavior pending. Strict reads and unproven cardinality rejected; defaulted reads require local proof of one well |
| A08 timing | F13 and primary APP SetTimer, Wait mode 0/2; [mapping](../../../autosuite/docs/26_TIMING_MAPPING.md) | 3.6.24 pp. 76–77; 3.7.12 pp. 101–102 | Implemented stage 10: five-second schedule, reset, ownership, definite starts, independent clocks and ordered events | Scope/reset across repeated calls, real elapsed waits, unchanged device state and manual-derived cancelwait=0 still pending. Dynamic durations and broader native timer scopes are explicitly rejected |
...
| A12 wall time | F15 DateTime format and Set Variable envelope; [mapping](../../../autosuite/docs/25_WALL_TIME_MAPPING.md) | 3.11.1 pp. 146–149 | Implemented stage 9: explicit aware clock, ordered single reads, immutable events, failed-read isolation | Local/DST time, calendar/year range, Unicode and escaped formats, actual read timing remain pending |
