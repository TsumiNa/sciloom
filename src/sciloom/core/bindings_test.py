...
def test_resolver_cannot_silently_drop_or_mistype_dependencies():
    source = replace(
        program(),
        resources=(DeviceResource(node_id="r", logical_id="mixer", device_type_id=AGITATOR_TYPE_ID),),
    ...
    wrong = replace(
        binding(), contract=BASE_DEVICE_CONTRACT, base_contracts=(), writable_properties=(), supported_operations=()
    )
    assert [d.code for d in validate_bindings(source, DeviceBindings(devices=(wrong,)))] == ["device_type"]
    assert [d.code for d in validate_bindings(program(), DeviceBindings(devices=(binding(),)))] == [
