...
def list_program():
    ...
    initial = ListLiteral(
        node_id="initial", type=kind, elements=(Literal(node_id="one", type=ScalarType.INTEGER, value=1),)
    )
    state = Variable(
    ...
    body = (
        Assignment(
            node_id="copy",
            target=Reference(node_id="out1", symbol_id="output"),
            value=Reference(node_id="state1", symbol_id="state"),
        ),
        ListSet(
            node_id="update",
            target=Reference(node_id="out2", symbol_id="output"),
            index=Literal(node_id="index", type=ScalarType.INTEGER, value=0),
            value=Literal(node_id="two", type=ScalarType.REAL, value=2.0),
            op=BinaryOp.MULTIPLY,
        ),
    )
    return Program(
        entry_function_id="f", functions=(FunctionIR(node_id="f", name="Lists", variables=(state, output), body=body),)
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda d: d["functions"][0]["variables"][0]["type"].update(
                element_type={"kind": "ListType", "element_type": "real"}
            ),
            "json_shape",
        ),
        ...
        (
            lambda d: d["functions"][0]["variables"][0]["initial"]["elements"][0].update(type="boolean", value=True),
            "list_element_type",
        ),
        (
            lambda d: d["functions"][0]["variables"][0]["initial"].update(
                elements=[{"kind": "Reference", "node_id": "dynamic", "symbol_id": "state"}]
            ),
            "initializer_literal",
        ),
    ],
...
def test_list_comparison_is_not_implicitly_python_list_comparison():
    ...
    comparison = Binary(
        node_id="equal",
        op=BinaryOp.EQUAL,
        left=Reference(node_id="left", symbol_id="state"),
        right=Reference(node_id="right", symbol_id="state"),
    )
...
def test_length_and_index_require_list_values():
    ...
    for value in (
        ...
        ListGet(
            node_id="get",
            value=Literal(node_id="scalar", type=ScalarType.INTEGER, value=1),
            index=Literal(node_id="index", type=ScalarType.INTEGER, value=0),
        ),
    ):
        invalid = replace(
            ...
            functions=(
                replace(
                    function,
                    body=(
                        Assignment(node_id="assign", target=Reference(node_id="out", symbol_id="output"), value=value),
                    ),
                ),
            ),
        )
...
def test_mutable_or_nested_typed_initializers_fail_structure_validation():
    ...
    for initial in (
        replace(variable.initial, elements=list(variable.initial.elements)),
        replace(variable.initial, type=ListType(element_type=ListType(element_type=ScalarType.REAL))),
    ):
        bad_variable = replace(variable, initial=initial)
        invalid = replace(
            program,
            functions=(replace(program.functions[0], variables=(bad_variable, program.functions[0].variables[1])),),
        )
