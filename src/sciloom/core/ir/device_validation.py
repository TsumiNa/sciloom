...
def validate_directory(program: Program, report: Report) -> None:
    ...
    for i, contract in enumerate(program.device_types):
        ...
        if contract.type_id in contract.base_type_ids or len(set(contract.base_type_ids)) != len(
            contract.base_type_ids
        ):
        ...
        if (
            ...
            or not set(contract.required_configuration) <= properties
        ):
    ...
    for contract in program.device_types:
        for base_id in contract.base_type_ids:
            ...
            if base is None or contract.type_id in base.base_type_ids:
                report(
                    "device_contract",
                    "Device ancestry must reference existing, acyclic type contracts.",
                    "$.device_types",
                    None,
                )
                ...
                report(
                    "device_contract",
                    "Device ancestry must include the complete base-type closure.",
                    "$.device_types",
                    None,
                )
            elif any(p not in contract.properties for p in base.properties) or any(
                op not in contract.operations for op in base.operations
            ):
                report(
                    "device_contract",
                    "Inherited member signatures cannot be removed or changed.",
                    "$.device_types",
                    None,
                )
...
def is_agitator(program: Program, resource: DeviceResource) -> bool:
    return any(
        c.type_id == resource.device_type_id and AGITATOR_TYPE_ID in (c.type_id, *c.base_type_ids)
