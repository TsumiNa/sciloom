...
from sciloom.core.ir import (
    Assignment, Binary, BinaryOp, Call, FunctionIR, InputBinding, ListGet,
    ListLength, ListLiteral, ListSet, ListType, Literal, OutputBinding, Program,
    Reference, ScalarType, Variable, VariableRole, from_json, to_json,
)
...
def io_program(element=ScalarType.REAL, body_factory=None):
    ...
    body = body_factory(n, kind) if body_factory else (n(Assignment, target=n(Reference, symbol_id="b"), value=n(Reference, symbol_id="a")),)
    return Program(entry_function_id="f", functions=(FunctionIR(node_id="f", name="Lists", variables=(source, result), body=body),))


@pytest.mark.parametrize("element,values,expected", [
...
@pytest.mark.parametrize("element,values", [
    (ScalarType.INTEGER, [True]), (ScalarType.INTEGER, [1.5]),
    (ScalarType.REAL, [False]), (ScalarType.REAL, [float("inf")]),
    (ScalarType.BOOLEAN, [1]), (ScalarType.REAL, [[1]]),
    (ScalarType.ROTATIONAL_SPEED, [1.0]), (ScalarType.REAL, [60 * rpm]),
...
def test_internal_list_persists_without_mutating_previous_snapshots_or_other_sessions():
    ...
    function = FunctionIR(node_id="f", name="Persistent", variables=(state, result), body=(
        n(ListSet, target=n(Reference, symbol_id="state"), index=n(Literal, type=ScalarType.INTEGER, value=0), value=n(Literal, type=ScalarType.INTEGER, value=1), op=BinaryOp.ADD),
...
def test_call_inputs_outputs_and_whole_assignments_do_not_alias():
    ...
    def set_at(id, value):
        return n(ListSet, target=ref(id), index=n(Literal, type=ScalarType.INTEGER, value=0), value=n(Literal, type=ScalarType.REAL, value=value))
    child = FunctionIR(node_id="child", name="MutateCopy", variables=(variable("x", "child", VariableRole.INPUT), variable("y", "child", VariableRole.OUTPUT)), body=(
        set_at("x", 9.0), n(Assignment, target=ref("y"), value=ref("x")),
    ))
    caller = FunctionIR(node_id="caller", name="Caller", variables=(variable("a", "caller", VariableRole.INPUT), *(variable(id, "caller", VariableRole.OUTPUT) for id in ("b", "original", "copy"))), body=(
        n(Call, function_id="child", inputs=(InputBinding(parameter_id="x", value=ref("a")),), outputs=(OutputBinding(parameter_id="y", target=ref("b")),)),
    ...
    assert Interpreter(program).run(inputs={"a": [1.0, 2.0]}).outputs == {"b": (11.0, 2.0), "copy": (9.0, 2.0), "original": (1.0, 2.0)}
...
def indexed_program(index, *, write=False, augmented=False, bad_rhs=False):
    ...
    if write:
        ...
        body = (n(ListSet, target=n(Reference, symbol_id="a"), index=index_node, value=rhs, op=BinaryOp.ADD if augmented else None), n(Assignment, target=n(Reference, symbol_id="b"), value=n(Literal, type=ScalarType.REAL, value=1.0)))
    else:
        body = (n(Assignment, target=n(Reference, symbol_id="b"), value=n(ListGet, value=n(Reference, symbol_id="a"), index=index_node)),)
    return Program(entry_function_id="f", functions=(FunctionIR(node_id="f", name="Index", variables=variables, body=body),)), index_node.node_id
...
def test_runtime_list_construction_and_length():
    def body(n, kind):
        return (n(Assignment, target=n(Reference, symbol_id="b"), value=n(ListLiteral, type=kind, elements=(n(ListLength, value=n(Reference, symbol_id="a")), n(ListGet, value=n(Reference, symbol_id="a"), index=n(Literal, type=ScalarType.INTEGER, value=0))))),)
